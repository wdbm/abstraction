...
# vector, expresses that word vector using sounds and then converts it to a    #
# natural language text expression by selecting from a set of existing         #
# dictionary of pairs of natural language expressions and word vectors.        #
#                                                                              #
...
def main(options):
    ...
    if engage_sounds:
        # Speak input expression.
        log.info("speak expression")
        os.system("echo {text} | festival --tts".format(
            text = expression
        ))
    ...
    if engage_sounds:
        # Express word vector using sounds.
        log.info("express word vector using sounds")
        datavision.play_message_sounds(
            message   = working_expression_WV
        )
    ...
    if abs(
        ...
        log.info("translation: {translation_expression_NL}".format(
            translation_expression_NL = translation_expression_NL
        ))
        if engage_sounds:
            # Speak translation expression.
            log.info("speak expression")
            os.system("echo {text} | festival --tts".format(
                text = translation_expression_NL
            ))
